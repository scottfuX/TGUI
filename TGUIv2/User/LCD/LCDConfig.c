 ...
 * @}
 */
 //在SDRAM中分配个这么大的连续地址 避免用直接地址访问 避免被其他程序串改
 uint8_t __EXRAM LCD_BUFADDR[BUFFER_OFFSET] ; 
#define LCD_FRAME_BUFFER  (uint32_t)LCD_BUFADDR;
...
void LCD_DrawFullRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height)
{
 ...
 /* Wait for CTC Flag activation *///当出现bug时会卡在这里
 while(DMA2D_GetFlagStatus(DMA2D_FLAG_TC) == RESET)
 {
 }
...
uint16_t RGB888ToRGB1555(uint32_t n888Color)  
{  
    uint16_t	n1555Color = 0;  
  
    // 鑾峰彇RGB鍗曡壊锛屽苟鎴彇楂樹綅  
    unsigned char cRed   = (n888Color & 0x00ff0000)  >> 19;  
    unsigned char cGreen = (n888Color & 0x0000ff00)  >> 11;  
    unsigned char cBlue  = (n888Color & 0x000000ff)  >> 3;  
  
    // 杩炴帴  
    n1555Color = 0x80000+(cRed << 10) + (cGreen << 5) + (cBlue << 0);  
    return n1555Color;  
}  
...
{  
    ...
    unsigned char cGreen = (n565Color & 0x07e0)  >> 3;  
