...
#define BUFFER_OFFSET          ((uint32_t)800*480*3)    //sdram为24位的,液晶RGB888
#define LCD_PIXCELS            ((uint32_t)800*480) 

//在SDRAM中分配个这么大的连续地址 避免用直接地址访问 避免被其他程序串改
static uint8_t  LCD_BUFADDR[BUFFER_OFFSET] __EXRAM ; 
...
//在SDRAM中分配个这么大的连续地址 避免用直接地址访问 避免被其他程序串改
//static uint8_t __EXRAM LCD_BUFADDR[BUFFER_OFFSET] ; 
//#define LCD_FRAME_BUFFER  (uint32_t)LCD_BUFADDR;
